...
def load_test(file, shader, light_dir, tex_file=None):
    """ load resources from file using assimp, return list of TexturedMesh """
    try:
        pp = assimpcy.aiPostProcessSteps
        flags = pp.aiProcess_Triangulate | pp.aiProcess_FlipUVs
    ...
    for mesh in scene.mMeshes:
        ...
        mesh = TestMesh(shader, mat['diffuse_map'], attributes, mesh.mFaces,
                        k_d=mat.get('COLOR_DIFFUSE', (1, 1, 1)),
                        k_s=mat.get('COLOR_SPECULAR', (1, 1, 1)),
                        k_a=mat.get('COLOR_AMBIENT', (0, 0, 0)),
                        s=mat.get('SHININESS', 16.),
...
def main():
    ...
    texture_shader = Shader("texture.vert", "texture.frag")
    test_shader = Shader("test.vert", "test.frag")
    ...
    # rabbit.add(*[mesh for mesh in load_textured(files[0], texture_shader)])
    ...
    # cube.add(rabbit_rotation, *[mesh for mesh in load_textured(files[1], texture_shader)])
    ...
    light_dir = vec(-1, 0, 0)
    cube.add(*[mesh for mesh in load_test("bunny/bunny.obj", test_shader, light_dir)])
